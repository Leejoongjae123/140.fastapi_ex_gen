...
@app.post('/search', response_model=List[Posting])
async def read_items(keywords1: Optional[List[str]] = None,keywords2: Optional[List[str]] = None):
    if keywords1 and keywords2:
        print("and_keyword")
        # 각각의 조건을 만들어줌
        conditions1 = [PostingTable.title.contains(keyword) for keyword in keywords1]
        combined_condition1 = or_(*conditions1)
        conditions2 = [PostingTable.title.contains(keyword) for keyword in keywords2]
        combined_condition2 = or_(*conditions2)
        postings = session.query(PostingTable).filter(and_(combined_condition1, combined_condition2))
